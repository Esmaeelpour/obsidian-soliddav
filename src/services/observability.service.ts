...
export default class ObservabilityService {
	...
	/** Last settled monitor-mode status text, so a running check can keep showing
	 * it (with a ⟳ marker) instead of flashing a separate "checking" message. */
	private lastMonitorText = '';
	...
	/** Monitor mode: a read-only check is running. Keep the current status visible
	 * with a ⟳ marker rather than flashing a separate "checking" message. */
	reportMonitorChecking() {
		this.stopTimeUpdates();
		const text = this.lastMonitorText ? `${this.lastMonitorText} ⟳` : 'Checking ⟳';
	...
	private updateStatusBarWithTime(): void {
		...
		if (diffSeconds < 60) {
			...
			this.plugin.ribbonManager.setStatusTooltip(this.baseStatusText);
		} else {
			...
			this.plugin.ribbonManager.setStatusTooltip(statusText);
		}
	}
	...
	private apply(run: SyncRunSnapshot) {
		...
		// In monitor mode the status is driven by reportMonitor* instead of the
		// normal sync-run snapshots, so don't let stray planning events touch it.
		if (this.plugin.settings.operatingMode === 'monitor') return;
		this.applyStatus(run);
		this.applyRibbonError(run);
		this.applyMobileSyncNotice(run);
		this.applyNotice(run, this.previousRun);
		this.applyProgressModal(run, this.previousRun);
	}

	/** Keep the ribbon icon in an error state after a failed run (cleared on the
	 * next sync/success) so the failure stays visible on mobile. */
	private applyRibbonError(run: SyncRunSnapshot) {
		if (!TERMINAL_STAGES.includes(run.stage)) {
			this.plugin.ribbonManager.setError(false);
			return;
		}
		const failed = run.stage === 'failed' || (run.resultSummary?.failedTasks ?? 0) > 0;
		this.plugin.ribbonManager.setError(failed);
	}

	private applyMobileSyncNotice(run: SyncRunSnapshot) {
		// No persistent/live notice on mobile. We only pop a brief, auto-dismissing
		// notice at the end of a run: always on failure; when the server actually
		// updated the vault; and — only if the user opted in — on every other sync.
		if (!Platform.isMobile || !TERMINAL_STAGES.includes(run.stage)) return;
		// A cancelled sync (manual stop, or app close/reopen) is not worth a notice.
		if (run.stage === 'cancelled') return;
		const failed = run.stage === 'failed' || (run.resultSummary?.failedTasks ?? 0) > 0;
		if (failed) {
			new Notice(this.getStatusText(run), 5000);
			return;
		}
		const updated = this.countVaultUpdatesFromServer(run);
		if (updated > 0) {
			new Notice(`WebDAV Sync: vault updated — ${updated} change(s) from server`, 4000);
			return;
		}
		if (this.plugin.settings.showSyncStatusInNotificationOnMobile)
			new Notice(this.getStatusText(run), MOBILE_SYNC_NOTICE_HIDE_DELAY);
	}

	/** How many completed tasks actually changed the local vault using server
	 * content (downloads, merges, local creates/removals driven by the remote). */
	private countVaultUpdatesFromServer(run: SyncRunSnapshot): number {
		const VAULT_UPDATING: ReadonlySet<string> = new Set([
			'download',
			'merge',
			'createLocalDir',
			'removeLocal',
		...
		return run.progressSummary.completed.filter((task) => VAULT_UPDATING.has(task.taskName))
			.length;
	}
	...
	private applyNotice(run: SyncRunSnapshot, previousRun?: SyncRunSnapshot) {
		...
		// On mobile, completion is shown via the ribbon + the brief terminal notice
		// in applyMobileSyncNotice — skip the generic desktop-style notice here.
